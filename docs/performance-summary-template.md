...
[상품 조회 캐시 테스트]
- 조건: 동일 productId={PRODUCT_ID}, {THREADS} threads, ramp-up {RAMP_UP}s, duration {DURATION}s
- 캐시 OFF: Throughput {OFF_THROUGHPUT}/sec, Avg {OFF_AVG_MS}ms, P95 {OFF_P95_MS}ms, P99 {OFF_P99_MS}ms
- 캐시 ON: Throughput {ON_THROUGHPUT}/sec, Avg {ON_AVG_MS}ms, P95 {ON_P95_MS}ms, P99 {ON_P99_MS}ms
- Grafana 관측: Redis commands/sec {REDIS_COMMANDS_NOTE}, 5xx 비율 {ERROR_RATIO}
- 해석: 동일 상품 반복 조회에서는 캐시 활성화 시 지연 지표가 개선되고, 읽기 트래픽 일부가 Redis로 이동하는 패턴을 확인했다.
```
...
```text
상품 조회 캐시 테스트에서 캐시 ON/OFF를 비교한 결과 P95가 {OFF_P95_MS}ms -> {ON_P95_MS}ms로 변해 캐시 적용 효과를 확인했다.
```
